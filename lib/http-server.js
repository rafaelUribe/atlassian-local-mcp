  ...
  const server = http.createServer((req, res) => {
    ...
    if (req.method === 'GET') {
      ...
      if (url.pathname === '/api/confluence/spaces') {
        return fetchAtlassian('/wiki/rest/api/space?limit=100&type=global', 'GET', null, (err, data) => {
          if (err) { res.writeHead(500, { 'Content-Type': 'application/json' }); return res.end(JSON.stringify({ error: err.message })); }
          const spaces = (data.results || []).filter(s => !s.key.startsWith('~')).map(s => ({ key: s.key, name: s.name }));
