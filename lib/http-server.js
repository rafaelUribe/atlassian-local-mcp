...
const { cacheAddLink, cacheRemoveLink, cacheRemoveTicket, cacheWrite, getDb, getDbConfig, saveDbConfig } = require('./cache');
const cfg = require('./config');
const { MAX_BODY_SIZE, ROOT } = cfg;
  ...
  const server = http.createServer((req, res) => {
    ...
    if (req.method === 'GET') {
      ...
      if (url.pathname === '/api/config') {
        try {
          ...
          const result = {
            JIRA_HOST:            cfg.ATLASSIAN_HOST,
            JIRA_EMAIL:           cfg.ATLASSIAN_EMAIL,
            JIRA_TOKEN:           cfg.ATLASSIAN_TOKEN ? '\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022' : '',
            BITBUCKET_WORKSPACE:  cfg.BITBUCKET_WORKSPACE,
            BITBUCKET_PROJECTS:   cfg.BITBUCKET_PROJECTS.join(','),
            CONFLUENCE_SPACES:    cfg.CONFLUENCE_SPACES.join(','),
            BASE_BRANCHES:        cfg.BASE_BRANCHES.join(','),
            BRANCH_PREFIX:        cfg.BRANCH_PREFIX,
            JIRA_EPIC:            cfg.JIRA_EPIC,
            HTTP_PORT:            String(cfg.HTTP_PORT),
          };
          res.writeHead(200, { 'Content-Type': 'application/json' });
      ...
      if (url.pathname === '/api/bitbucket/projects') {
        if (!cfg.BITBUCKET_WORKSPACE) { res.writeHead(200, { 'Content-Type': 'application/json' }); return res.end(JSON.stringify({ projects: [] })); }
        return fetchBitbucket(`/2.0/workspaces/${encodeURIComponent(cfg.BITBUCKET_WORKSPACE)}/projects?pagelen=50`, 'GET', null, (err, data) => {
          if (err) { res.writeHead(500, { 'Content-Type': 'application/json' }); return res.end(JSON.stringify({ error: err.message })); }
      ...
      if (url.pathname === '/api/jira/my-tickets') {
        const jql = `assignee = "${cfg.ATLASSIAN_EMAIL}" AND statusCategory != Done ORDER BY updated DESC`;
      ...
      if (url.pathname === '/api/bitbucket/repos') {
        if (!cfg.BITBUCKET_WORKSPACE) { res.writeHead(200, { 'Content-Type': 'application/json' }); return res.end('[]'); }
        const paramProjects = url.searchParams.get('projects');
        const activeProjects = paramProjects
          ? paramProjects.split(',').map(s => s.trim()).filter(Boolean)
          : cfg.BITBUCKET_PROJECTS;
        let qFilter = '';
        if (activeProjects.length === 1) {
          qFilter = `&q=project.key%3D%22${encodeURIComponent(activeProjects[0])}%22`;
        } else if (activeProjects.length > 1) {
          const keys = activeProjects.map(k => `%22${encodeURIComponent(k)}%22`).join('%2C');
          qFilter = `&q=project.key+IN+(${keys})`;
        }
        return fetchBitbucket(`/2.0/repositories/${encodeURIComponent(cfg.BITBUCKET_WORKSPACE)}?pagelen=100${qFilter}`, 'GET', null, (err, data) => {
    ...
    if (url.pathname === '/api/config') {
      return readBody(req, res, (data) => {
        const allowed = ['JIRA_HOST', 'JIRA_EMAIL', 'JIRA_TOKEN', 'BITBUCKET_WORKSPACE', 'BITBUCKET_PROJECTS',
                         'BASE_BRANCHES', 'BRANCH_PREFIX', 'CONFLUENCE_SPACES', 'JIRA_EPIC', 'HTTP_PORT', 'HTTP_BIND'];
        const toSave = {};
        for (const key of allowed) {
          if (data[key] !== undefined) {
            const val = String(data[key] || '').trim();
            if (key === 'JIRA_TOKEN' && (val === '\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022' || val === '')) continue;
            toSave[key] = val;
          }
        }
        saveDbConfig(toSave);
        cfg.applyDbConfig(toSave);
        res.writeHead(200, { 'Content-Type': 'application/json' });
