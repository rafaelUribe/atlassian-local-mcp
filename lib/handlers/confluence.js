...
module.exports = {
  ...
  confluence_get_spaces: (id, args, send, sendErr) => {
    fetchAtlassian('/wiki/rest/api/space?limit=50&expand=description.plain', 'GET', null, (err, res) => {
      if (err) return sendErr(id, err);
      const spaces = res.results || [];
      if (!spaces.length) return send(id, 'No se encontraron espacios.');
      const lines = spaces.map(s =>
        `[${s.key}] ${s.name}${s.description?.plain?.value ? ` — ${s.description.plain.value.slice(0, 80)}` : ''}`
      );
      send(id, `Espacios (${lines.length}):\n\n${lines.join('\n')}`);
    });
  },
