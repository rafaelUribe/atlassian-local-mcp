...
module.exports = {
  ...
  bitbucket_get_repos: (id, args, send, sendErr) => {
    if (!cfg.BITBUCKET_WORKSPACE) return send(id, 'Bitbucket workspace not configured');
    let apiPath = `/2.0/repositories/${encodeURIComponent(cfg.BITBUCKET_WORKSPACE)}?pagelen=50&sort=-updated_on`;
    ...
    fetchBitbucket(apiPath, 'GET', null, (err, res) => {
      ...
      send(id, `Repositories en ${cfg.BITBUCKET_WORKSPACE} (${repos.length}):\n\n${lines.join('\n')}`);
    });
  },
  ...
  bitbucket_get_pull_requests: (id, args, send, sendErr) => {
    ...
    const apiPath = `/2.0/repositories/${encodeURIComponent(cfg.BITBUCKET_WORKSPACE)}/${encodeURIComponent(args.repo)}/pullrequests?state=${state}&pagelen=20`;
    fetchBitbucket(apiPath, 'GET', null, (err, res) => {
      if (err) return sendErr(id, err);
  ...
  bitbucket_get_pull_request: (id, args, send, sendErr) => {
    if (!cfg.BITBUCKET_WORKSPACE) return send(id, 'Bitbucket workspace not configured');
    const apiPath = `/2.0/repositories/${encodeURIComponent(cfg.BITBUCKET_WORKSPACE)}/${encodeURIComponent(args.repo)}/pullrequests/${args.prId}`;
  ...
  bitbucket_create_pull_request: (id, args, send, sendErr) => {
    if (!cfg.BITBUCKET_WORKSPACE) return send(id, 'Bitbucket workspace not configured');
    const apiPath = `/2.0/repositories/${encodeURIComponent(cfg.BITBUCKET_WORKSPACE)}/${encodeURIComponent(args.repo)}/pullrequests`;
    const body = {
  ...
  bitbucket_add_pr_comment: (id, args, send, sendErr) => {
    if (!cfg.BITBUCKET_WORKSPACE) return send(id, 'Bitbucket workspace not configured');
    const apiPath = `/2.0/repositories/${encodeURIComponent(cfg.BITBUCKET_WORKSPACE)}/${encodeURIComponent(args.repo)}/pullrequests/${args.prId}/comments`;
  ...
  bitbucket_get_branches: (id, args, send, sendErr) => {
    if (!cfg.BITBUCKET_WORKSPACE) return send(id, 'Bitbucket workspace not configured');
    let apiPath = `/2.0/repositories/${encodeURIComponent(cfg.BITBUCKET_WORKSPACE)}/${encodeURIComponent(args.repo)}/refs/branches?pagelen=30&sort=-target.date`;
    if (args.filter) apiPath += `&q=name~"${encodeURIComponent(args.filter)}"`;
    fetchBitbucket(apiPath, 'GET', null, (err, res) => {
      if (err) return sendErr(id, err);
  ...
  bitbucket_get_commits: (id, args, send, sendErr) => {
    ...
    let apiPath = `/2.0/repositories/${encodeURIComponent(cfg.BITBUCKET_WORKSPACE)}/${encodeURIComponent(args.repo)}/commits?pagelen=${limit}`;
  ...
  bitbucket_get_diff: (id, args, send, sendErr) => {
    if (!cfg.BITBUCKET_WORKSPACE) return send(id, 'Bitbucket workspace not configured');
    const apiPath = `/2.0/repositories/${encodeURIComponent(cfg.BITBUCKET_WORKSPACE)}/${encodeURIComponent(args.repo)}/pullrequests/${args.prId}/diff`;
    fetchBitbucket(apiPath, 'GET', null, (err, res) => {
      if (err) return sendErr(id, err);
  ...
  bitbucket_approve_pr: (id, args, send, sendErr) => {
    if (!cfg.BITBUCKET_WORKSPACE) return send(id, 'Bitbucket workspace not configured');
    const apiPath = `/2.0/repositories/${encodeURIComponent(cfg.BITBUCKET_WORKSPACE)}/${encodeURIComponent(args.repo)}/pullrequests/${args.prId}/approve`;
  ...
  bitbucket_merge_pr: (id, args, send, sendErr) => {
    if (!cfg.BITBUCKET_WORKSPACE) return send(id, 'Bitbucket workspace not configured');
    const apiPath = `/2.0/repositories/${encodeURIComponent(cfg.BITBUCKET_WORKSPACE)}/${encodeURIComponent(args.repo)}/pullrequests/${args.prId}/merge`;
  ...
  bitbucket_decline_pr: (id, args, send, sendErr) => {
    if (!cfg.BITBUCKET_WORKSPACE) return send(id, 'Bitbucket workspace not configured');
    const apiPath = `/2.0/repositories/${encodeURIComponent(cfg.BITBUCKET_WORKSPACE)}/${encodeURIComponent(args.repo)}/pullrequests/${args.prId}/decline`;
    fetchBitbucket(apiPath, 'POST', {}, (err) => {
      if (err) return sendErr(id, err);
  ...
  bitbucket_get_pr_comments: (id, args, send, sendErr) => {
    if (!cfg.BITBUCKET_WORKSPACE) return send(id, 'Bitbucket workspace not configured');
    const apiPath = `/2.0/repositories/${encodeURIComponent(cfg.BITBUCKET_WORKSPACE)}/${encodeURIComponent(args.repo)}/pullrequests/${args.prId}/comments?pagelen=50`;
    fetchBitbucket(apiPath, 'GET', null, (err, res) => {
      if (err) return sendErr(id, err);
  ...
  bitbucket_get_file: (id, args, send, sendErr) => {
    ...
    const apiPath  = `/2.0/repositories/${encodeURIComponent(cfg.BITBUCKET_WORKSPACE)}/${encodeURIComponent(args.repo)}/src/HEAD/${encodeURIComponent(args.path)}${branchQs}`;
    fetchBitbucket(apiPath, 'GET', null, (err, res) => {
      if (err) return sendErr(id, err);
      const text = typeof res === 'string' ? res : JSON.stringify(res);
  ...
  bitbucket_get_pipelines: (id, args, send, sendErr) => {
    ...
    const apiPath = `/2.0/repositories/${encodeURIComponent(cfg.BITBUCKET_WORKSPACE)}/${encodeURIComponent(args.repo)}/pipelines/?sort=-created_on&pagelen=${limit}`;
    fetchBitbucket(apiPath, 'GET', null, (err, res) => {
      if (err) return sendErr(id, err);
  ...
  bitbucket_get_pipeline: (id, args, send, sendErr) => {
    if (!cfg.BITBUCKET_WORKSPACE) return send(id, 'Bitbucket workspace not configured');
    const apiPath = `/2.0/repositories/${encodeURIComponent(cfg.BITBUCKET_WORKSPACE)}/${encodeURIComponent(args.repo)}/pipelines/${encodeURIComponent(args.pipelineId)}`;
  ...
  bitbucket_list_workspace_members: (id, args, send, sendErr) => {
    if (!cfg.BITBUCKET_WORKSPACE) return send(id, 'Bitbucket workspace not configured');
    const apiPath = `/2.0/workspaces/${encodeURIComponent(cfg.BITBUCKET_WORKSPACE)}/members?pagelen=50`;
    fetchBitbucket(apiPath, 'GET', null, (err, res) => {
      if (err) return sendErr(id, err);
      ...
      send(id, `Workspace members ${cfg.BITBUCKET_WORKSPACE} (${lines.length}):\n\n${lines.join('\n')}`);
    });
  },
